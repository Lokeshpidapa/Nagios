...
# Copyright (C) 2016 Steven Beauchemin
# Copyright (C) 2014 Michael Graves
...
#
#	2016-10-10 - added redis support so the test can be farmed out to gearman hosts - SLB
#	  -R redisMaster:port,redisSlave:port
#	  -R 11.22.33.44:7001,localhost:7002
#
#
#
#
...
# Path to  tmp files
my $TRAFFIC_FILE = "/var/tmp/traffic/";
...
# # Redis Added 2016-10-10 - SLB
# Use redis instead of tmp files
my $redis;              # use redis and not a traffic file
my $redisttl = "86400"; # default redis TTL - remove entry after this many seconds - one day
my $redismaster;        # write to here
my $redisslave;         # read from here
...
my $status = GetOptions(
	...
	"d=s"		=> \$authpass,	"authpass=s"	=> \$authpass,
	"h|help"        => \$opt_h,
	...
	"p|port=i"      => \$port,
	"r"		=> \$use_reg,	"noregexp"	=> \$use_reg,
	"S"	  	=> \$index_list, "list"		=> \$index_list,
	"u|units=s"     => \$units,

	# Added 2016-10-10 - SLB
	"R|redis=s"     => \$redis,
...
# Added 2016-10-10 - SLB - Support redis server instead of using /tmp files
# redis parameter should be like this -> master:port,slave:port,TTL
#                                          1   : 2  ,  3  : 4    5
if ($redis) {
  if ( $redis =~ m/(.*):(\d+),(.*):(\d+),(\d+)/ ) { # with ttl
    use Redis::Client;
    $redismaster = Redis::Client->new( host => $1, port => $2 )
      or die "Can't open redis master for writing: $!\n";
    $redisslave = Redis::Client->new( host => $3, port => $4 )
      or die "Can't open redis slave for reading $!\n";
    $redisttl = $5;
  } elsif ( $redis =~ m/(.*):(\d+),(.*):(\d+)/ ) { # no ttl provided using default
    use Redis::Client;
    $redismaster = Redis::Client->new( host => $1, port => $2 )
      or die "Can't open redis master for writing: $!\n";
    $redisslave = Redis::Client->new( host => $3, port => $4 )
    ...
    print "redis syntax needs to be as follows\n";
    print "[master:port,slave:port,TTL]\n";
    print "example. 111.222.333.444:7001,localhost:7001,1200\n";
    print "or [master:port,slave:port] - will use default ttl of $redisttl\n";
    print "example. 111.222.333.444:7001,localhost:7001\n\n\n";
    &print_usage();
...
# mg - adding snmp v3 support
    # Must define a security level even though default is noAuthNoPriv
    # v3 requires a security username
    if (defined $seclevel  && defined $secname) {

      # Must define a security level even though defualt is noAuthNoPriv
      if (!(($seclevel eq 'noAuthNoPriv') || ($seclevel eq 'authNoPriv') || ($seclevel eq 'authPriv' )) ) {
        stop("must select a security level '".$seclevel."'\n","UNKNOWN");
      }

      # Authentication wanted
      if ($seclevel eq ('authNoPriv' || 'authPriv') ) {

        if (!(($authproto eq 'MD5') || ($authproto eq 'SHA1')) ) {
          stop("must select an authproto '".$authproto."'\n","UNKNOWN");
        }

        if ( !defined $authpass) {
          stop("must specify an authpass\n","UNKNOWN");
        } else {
          if ($authpass =~ /^0x/ ) {
      ...
      unless ( defined $context) {
		...
		stop("must specify a secname and seclevel\n","UNKNOWN");
  }
...
#      $priv
  ...
  if (!defined($session)) {
		stop("unable to establish session '".$error."'\n","UNKNOWN");
  }
} else {
	$state = 'UNKNOWN';
...
# Added 2016-10-10 - SLB - Support redis server instead of using /tmp files
# also reformatted some lines so I could understand the code better
# this is the read file or redis slave piece to see previous data
if ($redis) {
  my $rediskey = $host_address . "_if-" . $iface_number;
  $row = $redisslave->get( $rediskey )
    or print "No data yet for $rediskey  - run the test again $!\n";
  ...
  if ( open( FILE, "<" . $TRAFFIC_FILE . $host_address . "_if-" . $iface_number)) {
    while ( $row = <FILE> ) {
...
# end redis read from slave and read from tmp file

my $update_time = time;

# Added 2016-10-10 - SLB - Support redis server instead of using /tmp files
# this is the write file or redis master piece to save data
if ($redis) {
  my $rediskey = $host_address . "_if-" . $iface_number;
  my $redisvalue = sprintf("%s:%.0ld:%.0ld\n", $update_time, $in_bytes, $out_bytes);
  $redismaster->setex( $rediskey, $redisttl, $redisvalue )   # goes bye-bye in TTL
    or die "Can't open redis master for writing: $!\n";
} else {
...
# end redis write to master and write to tmp file
...
if ( $if_status != 1 ) {
	...
	$output  = "$state - ";
	# only display the next line if the -A parameter is asking for specific IP
	$output .= "IP " . $host_ip . ", " if ($host_ip);
	$output .= "Average IN: " . $in_traffic . $in_prefix . $suffix . " (" . $in_usage . "%), ";
	$output .= "Average OUT: " . $out_traffic . $out_prefix . $suffix . " (" . $out_usage . "%) <br>";
...
sub print_usage {
	print <<EOU;
	Usage: check_iftraffic.pl -H host [ -C community_string ] [ -i if_index|if_descr ] [ -r ] [ -b if_max_speed_in | -I if_max_speed_in ] [ -O if_max_speed_out ] [ -u ] [ -B ] [ -A IP Address ] [ -L ] [ -M ] [ -w warn ] [ -c crit ] [ -R master:port,slave:port,TTL ]
	...
	Example 7: check_iftraffic.pl -H host1 -C sneaky -A 192.168.1.1 -B -b 100 -u m -R 11.22.33.44:6379,localhost:6379,900
	...
	-x (--context)      SNMPv3 context name (default is empty  string)
	-d (--authpass)     authentication password (cleartext ascii or localized key
	...
	-R, --redis master:port,slave:port,[optional Time-To-Live-seconds]
		Tell plugin to use redis for saving state data versus using the /tmp file.
		This is useful when you have redis installed and also desire to use
		mod_gearman and run tests remote from your nagios core system. The state
		data acquired from counters will be written to and read from the redis system
		and be available and accurate no matter which random gearman host runs the test.
		The TTL will remove the entry from redis if not reset in a reasonable test cycle time.
EOU
